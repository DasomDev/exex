...
<template>
  <div
    class="fixed right-0 rounded-r-none rounded-l-2xl top-1/2 z-10 p-6 bg-white shadow-lg transform -translate-y-1/2 w-fit h-fit"
  >
            ...
            :class="[
              'w-8 h-8 rounded-full text-xs font-medium transition-all duration-200 hover:scale-110',
              {
                'bg-gray-200 text-gray-600 hover:bg-gray-300':
                  getQuestionStatus(sectionIndex, questionIndex) === 'unanswered',
                'bg-green-500 text-white hover:bg-green-600':
                  getQuestionStatus(sectionIndex, questionIndex) === 'correct',
                'bg-red-500 text-white hover:bg-red-600':
                  getQuestionStatus(sectionIndex, questionIndex) === 'incorrect',
              },
            ]"
